    ...
    def _schedule_async_pre_pull(self):
        ...
        def async_pre_pull():
            try:
                ...
                # Quick check if image is already available (should be from build-time save/load)
                try:
                    docker_client = docker.from_env()
                    docker_client.images.get(AGENT_SERVER_IMAGE)
                    logger.info(f"✅ Agent server image {AGENT_SERVER_IMAGE} already available (likely from build-time save/load)")
                    return
                except docker.errors.ImageNotFound:
