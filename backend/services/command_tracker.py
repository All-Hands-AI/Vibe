...
class CommandTracker:
    ...
    def track_command_sent(
        ...
        try:
            ...
            command_id = (
                f"{action}_{int(datetime.now(timezone.utc).timestamp() * 1000)}"
            )

            # Initialize action tracking if not exists
            if action not in commands:
                commands[action] = {"latest_command_id": None, "commands": []}
                ...
                if action in commands and commands[action]["commands"]:
                    ...
                    for command_record in reversed(commands[action]["commands"]):
                        if command_record["status"] == "sent" and (
                            not command_record.get("tool_call_id")
                            or command_record.get("tool_call_id") == tool_call_id
                        ):
                            ...
                            if event_type == "ObservationEvent" and event_data:
                                ...
                                command_record["completed_at"] = datetime.now(
                                    timezone.utc
                                ).isoformat()
                                command_record["exit_code"] = event_data.get(
                                    "exit_code"
                                )
                                command_record["output"] = event_data.get("output", "")[
                                    :1000
                                ]  # Limit output size
    ...
    def get_latest_command_id(
        self, user_uuid: str, app_slug: str, riff_slug: str, action: str
    ) -> Optional[str]:
        """
    ...
    def get_command_status(
        ...
        try:
            ...
            return {
                ...
                "output": (
                    latest_command.get("output", "")[:200] + "..."
                    if latest_command.get("output", "")
                    else ""
                ),
            }
    ...
    def get_all_commands_status(
        ...
        try:
            result = {}
            for action in ["install", "run", "test", "lint"]:
                result[action] = self.get_command_status(
                    user_uuid, app_slug, riff_slug, action
                )
            return result
        except Exception as e:
