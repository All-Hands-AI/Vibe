...
class RuntimeService:
    ...
    def __init__(self):
        ...
        if not self.admin_api_key:
            logger.warning("⚠️ RUNTIME_API_STAGING_SECRET not set - runtime operations will fail")
    ...
    def start_runtime(self, user_uuid: str, app_slug: str, riff_slug: str) -> Tuple[bool, Dict]:
        """
        ...
        if success:
            logger.info(f"✅ Runtime started successfully: {response.get('runtime_id')}")
        else:
            logger.error(f"❌ Failed to start runtime: {response.get('error')}")
        
        return success, response
    
    def get_runtime_status(self, user_uuid: str, app_slug: str, riff_slug: str) -> Tuple[bool, Dict]:
        """
    ...
    def get_api_health(self) -> Tuple[bool, Dict]:
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                logger.warning(f"⚠️ Runtime API health check failed: {response.status_code}")
    ...
    def handle_agent_reset(self, user_uuid: str, app_slug: str, riff_slug: str) -> Tuple[bool, Dict]:
        """
        ...
        Returns:
            Tuple of (success: bool, response_data: Dict)
        """
        logger.info(f"🔄 Handling agent reset for {user_uuid[:8]}:{app_slug}:{riff_slug}")
        
        # First, check the current runtime status
        success, status_response = self.get_runtime_status(user_uuid, app_slug, riff_slug)
        ...
        if current_status == 'paused' and runtime_id:
            ...
            logger.info(f"🆕 Runtime status is '{current_status}', starting new runtime")
            return self.start_runtime(user_uuid, app_slug, riff_slug)
