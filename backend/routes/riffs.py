...
@riffs_bp.route("/api/apps/<slug>/riffs/<riff_slug>/status", methods=["GET"])
def get_riff_status(slug, riff_slug):
    """Get comprehensive status for a riff including PR, CI, agent, and commit info"""
    logger.info(
        f"📊 GET /api/apps/{slug}/riffs/{riff_slug}/status - Getting comprehensive riff status"
    )
    ...
    try:
        ...
        # Get PR status if GitHub is configured
        if github_url and github_token:
            logger.info(f"🔍 Getting PR status for riff '{riff_slug}'")
            pr_status = get_pr_status(github_url, github_token, riff_slug)
            if pr_status:
                status_response["pr_status"] = pr_status
                status_response["ci_status"] = pr_status.get("ci_status")
                ...
                logger.info(f"ℹ️ No PR found for riff {riff_slug}")

        # Get agent status
        try:
            agent_loop = agent_loop_manager.get_agent_loop(user_uuid, slug, riff_slug)
            if agent_loop:
                agent_status = agent_loop.get_status()
            ...
            logger.warning(f"⚠️ Error getting agent status for riff {riff_slug}: {e}")
            status_response["agent_status"] = {
                "status": "error",
                "is_running": False,
                "is_paused": False,
        ...
        logger.error(f"💥 Error getting riff status: {str(e)}")
        return jsonify({"error": "Failed to get riff status"}), 500
