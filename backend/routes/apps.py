...
def create_app():
    ...
    try:
        ...
        logger.info(f"🔄 Creating app: {app_name} -> {app_slug} for user {user_uuid[:8]}")
        
        # Check if app with same slug already exists for this user
        if user_app_exists(user_uuid, app_slug):
            logger.warning(f"❌ App with slug '{app_slug}' already exists for user {user_uuid[:8]}")
...
def delete_app(slug):
    ...
    try:
        ...
        if delete_user_app(user_uuid, slug):
            logger.info(f"✅ App {slug} and all associated data deleted for user {user_uuid[:8]}")
        else:
