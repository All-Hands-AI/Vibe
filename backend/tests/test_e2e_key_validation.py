    ...
    def test_key_validation_error_handling(self):
        ...
        for invalid_input in invalid_inputs:
            ...
            try:
                ...
                pytest.fail(f"validate_anthropic_key raised exception for {invalid_input}: {e}")
            
            try:
                result = validate_github_key(invalid_input)
                assert result is False, f"Expected False for input: {invalid_input}"
            except Exception as e:
                pytest.fail(f"validate_github_key raised exception for {invalid_input}: {e}")
            
            try:
                result = validate_fly_key(invalid_input)
                assert result is False, f"Expected False for input: {invalid_input}"
            except Exception as e:
                pytest.fail(f"validate_fly_key raised exception for {invalid_input}: {e}")
