...
def mock_repository_setup(monkeypatch, temp_data_dir):
    ...
    def mock_setup_riff_workspace(user_uuid, app_slug, riff_slug, github_url):
        ...
        workspace_path = f"{temp_data_dir}/{user_uuid}/apps/{app_slug}/riffs/{riff_slug}/workspace"
    ...
    def mock_create_agent_loop(user_uuid, app_slug, riff_slug, llm, workspace_path, message_callback=None):
        """Mock AgentLoop creation that doesn't actually create tools"""
        class MockAgentLoop:
            def __init__(self, user_uuid, app_slug, riff_slug, llm, workspace_path, message_callback=None):
        ...
        return MockAgentLoop(user_uuid, app_slug, riff_slug, llm, workspace_path, message_callback)
    
    # Mock the agent_loop_manager.create_agent_loop method
    monkeypatch.setattr("routes.riffs.agent_loop_manager.create_agent_loop", mock_create_agent_loop)
    ...
    def mock_get_agent_loop(user_uuid, app_slug, riff_slug):
        ...
        return mock_create_agent_loop(user_uuid, app_slug, riff_slug, None, f"/mock/workspace/{user_uuid}/{app_slug}/{riff_slug}")
    
    monkeypatch.setattr("routes.riffs.agent_loop_manager.get_agent_loop", mock_get_agent_loop)
