...
def get_deployment_status(repo_url, github_token, branch_name):
    """
    Get deployment status by checking GitHub Actions for "Deploy app" job.
    ...
    """
    logger.info(f"🚀 Checking deployment status for branch '{branch_name}' in {repo_url}")
    ...
    try:
        ...
        if branch_response.status_code == 404:
            ...
            logger.warning(f"❌ Failed to get branch info: {branch_response.status_code}")
            return {
                "status": "error",
        ...
        if runs_response.status_code != 200:
            logger.warning(f"❌ Failed to get workflow runs: {runs_response.status_code}")
            return {
                "status": "error",
        ...
        logger.info(f"🔍 Found {len(workflow_runs)} workflow runs for commit {commit_sha[:8]}")
        
        # Step 3: Look for "Deploy app" job in the workflow runs
        deploy_run = None
        for run in workflow_runs:
            run_name = run.get("name", "")
            if "Deploy app" in run_name:
                deploy_run = run
                logger.info(f"✅ Found 'Deploy app' workflow: {run_name}")
                break
        
        if not deploy_run:
            logger.info(f"❌ No 'Deploy app' workflow found for commit {commit_sha[:8]}")
            return {
                "status": "error",
                "message": f"No 'Deploy app' workflow found for commit {commit_sha[:8]}",
                "details": {
                    "error": "deploy_workflow_not_found",
                    "commit_sha": commit_sha,
                    "available_workflows": [run.get("name", "unnamed") for run in workflow_runs[:5]]
                }
            }
        
        # Step 4: Check the status of the deploy workflow
        run_status = deploy_run.get("status")  # queued, in_progress, completed
        run_conclusion = deploy_run.get("conclusion")  # success, failure, cancelled, etc.
        run_url = deploy_run.get("html_url")
        
        logger.info(f"🔍 Deploy workflow status: {run_status}, conclusion: {run_conclusion}")
